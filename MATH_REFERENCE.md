# BCE Engine — Mathematical Reference
...
The BCE (Ballistic Core Engine) is a single-instance, zero-heap-allocation C++ engine. It is called once per sensor frame via `BCE_Update()` and outputs a `FiringSolution` containing MOA hold values, ballistic data, and uncertainty estimates.
...
**Sync note:** This diagram mirrors the `BCE_Engine::update()` and `BCE_Engine::computeSolution()` call graph in [lib/bce/src/engine/bce_engine.cpp](lib/bce/src/engine/bce_engine.cpp).

---

## 2. Coordinate System & Sign Conventions

The trajectory integrator ([lib/bce/src/solver/solver.cpp](lib/bce/src/solver/solver.cpp)) uses a right-handed, bore-centred frame:
...
**Source:** [lib/bce/src/atmo/atmosphere.cpp](lib/bce/src/atmo/atmosphere.cpp), `Atmosphere::recompute()`.
...
**Source:** [lib/bce/src/atmo/atmosphere.cpp](lib/bce/src/atmo/atmosphere.cpp), `Atmosphere::correctBC()`.
...
**Source:** [lib/bce/src/drag/drag_model.cpp](lib/bce/src/drag/drag_model.cpp), [lib/bce/src/drag/drag_tables.h](lib/bce/src/drag/drag_tables.h).
...
- $\rho_{std} = 1.225$ kg/m³ (ISA sea-level standard, `BCE_STD_AIR_DENSITY`)
- $v$ = bullet speed relative to surrounding air (m/s)
- $BC_{corrected}$ = atmospherically-corrected ballistic coefficient (from §4.5)
- $K_{bal} = 900$ (`BCE_BALLISTIC_DRAG_CONSTANT`) — the legacy ballistic drag constant that maps BC (lbf/in²) to SI retardation units
...
> **Sync:** If $K_{bal}$, $\rho_{std}$, or the formula structure changes, update §5.2 and `BCE_BALLISTIC_DRAG_CONSTANT` in `bce_config.h`.
...
**Source:** [lib/bce/src/ahrs/madgwick.cpp](lib/bce/src/ahrs/madgwick.cpp), [lib/bce/src/ahrs/mahony.cpp](lib/bce/src/ahrs/mahony.cpp).
...
where $\beta = 0.1$ (default, `BCE_MADGWICK_DEFAULT_BETA`) controls the aggressiveness of the correction (larger $\beta$ → trusts accelerometer/magnetometer more, less gyro drift, more noise).

> **Sync:** If `BCE_MADGWICK_DEFAULT_BETA` changes, update this section.
...
where:
- $k_p = 2.0$ (`BCE_MAHONY_DEFAULT_KP`) — proportional gain
- $k_i = 0.005$ (`BCE_MAHONY_DEFAULT_KI`) — integral gain (removes gyro bias drift)

The corrected $\boldsymbol{\omega}_{corrected}$ is then used in the quaternion kinematics of §6.1 → §6.5.

> **Sync:** If $k_p$ or $k_i$ defaults change, update §6.9 and the config constants.
...
**Source:** [lib/bce/src/ahrs/ahrs_interface.h](lib/bce/src/ahrs/ahrs_interface.h).
...
**Source:** [lib/bce/src/solver/solver.cpp](lib/bce/src/solver/solver.cpp), `BallisticSolver::solveZeroAngle()`.
...
Wait — that's relative to the bore. The bullet starts at bore ($y = 0$), arcs up then down, and must arrive at $y = -h_s$ at range $R_0$ to intersect a LOS that is $h_s$ above the bore at $x = 0$ and on-target at $x = R_0$.
...
3. If $y(\text{at } R_0) > -h_s$: bullet is too high → $\theta_{hi} = \theta_{mid}$
4. If $y(\text{at } R_0) < -h_s$: bullet is too low → $\theta_{lo} = \theta_{mid}$
...
**Source:** [lib/bce/src/engine/bce_engine.cpp](lib/bce/src/engine/bce_engine.cpp), `BCE_Engine::buildSolverParams()`.
...
> **Implementation:** `bce_engine.cpp` lines tagged `[MATH §9]`

---

## 10. Wind Decomposition

**Source:** [lib/bce/src/corrections/wind.cpp](lib/bce/src/corrections/wind.cpp), `WindCorrection::decompose()`.
...
**Source:** [lib/bce/src/solver/solver.cpp](lib/bce/src/solver/solver.cpp), `BallisticSolver::integrateToRange()`.
...
> **Sync:** If gravity constant changes, update §11.3 and `BCE_GRAVITY` in `bce_config.h`.
...
> **Sync:** If any of the adaptive timestep constants change, update §11.4 and the corresponding `BCE_DT_*` / `BCE_MAX_STEP_DISTANCE_M` constants in `bce_config.h`.
...
- $|\mathbf{v}| < 30$ m/s (`BCE_MIN_VELOCITY`) — bullet subsided
- Iterations $\ge 500{,}000$ (`BCE_MAX_SOLVER_ITERATIONS`) — runaway guard
...
**Source:** [lib/bce/src/solver/solver.cpp](lib/bce/src/solver/solver.cpp), `BallisticSolver::integrate()`.
...
In the Northern hemisphere ($\lambda > 0$), $\Delta z > 0$ → bullet deflects right. In the Southern hemisphere, bullets deflect left.
...
**Source:** [lib/bce/src/engine/bce_engine.cpp](lib/bce/src/engine/bce_engine.cpp), `BCE_Engine::computeSolution()`.
...
where $\alpha = 0.2$ (`BCE_LRF_FILTER_ALPHA`). This weights the new reading at 20% and retains 80% of the history, providing modest noise rejection while remaining responsive.

> **Sync:** If $\alpha$ changes, update §14.3 and `BCE_LRF_FILTER_ALPHA` in `bce_engine.cpp`.
> **Implementation:** `bce_engine.cpp` lines tagged `[MATH §14.3]`
...
Constants 3.1435 mm and 2.3560 mm are the half-width and half-height of the IMX477 sensor (`BCE_SENSOR_HALF_WIDTH_MM`, `BCE_SENSOR_HALF_HEIGHT_MM`).

> **Sync:** If the camera sensor changes, update both constants in `bce_config.h` and §14.4.
> **Implementation:** `bce_engine.cpp` lines tagged `[MATH §14.4]`
...
> **Implementation:** `bce_engine.cpp` lines tagged `[MATH §14.1]`–`[MATH §14.4]`

---

## 15. Cant Correction

**Source:** [lib/bce/src/corrections/cant.cpp](lib/bce/src/corrections/cant.cpp), `CantCorrection::apply()`.
...
> **Sync:** If the cant model changes (e.g., cant is applied before the other offsets), update both §15 and the call site order in `bce_engine.cpp` `computeSolution()`.
> **Implementation:** `cant.cpp` lines tagged `[MATH §15]`; call site in `bce_engine.cpp` tagged `[MATH §15]`

---

## 16. Uncertainty Propagation

**Source:** [lib/bce/src/engine/bce_engine.cpp](lib/bce/src/engine/bce_engine.cpp), `BCE_Engine::computeUncertainty()`.
...
> **Sync:** If the number of uncertain inputs changes, update the `kNumUncertaintyInputs` constant in `bce_types.h` and the index table below.
> **Implementation:** `bce_engine.cpp` lines tagged `[MATH §16.1]`
...
> **Implementation:** `bce_engine.cpp` lines tagged `[MATH §16.2]`
...
> **Sync:** If default sigma values change, update the table above and `getDefaultUncertaintyConfig()` in `bce_engine.cpp`.

---

## Appendix A — Constants & Unit Reference

**Source:** [lib/bce/include/bce/bce_config.h](lib/bce/include/bce/bce_config.h), [lib/bce/include/bce/bce_math_utils.h](lib/bce/include/bce/bce_math_utils.h).

### A.1 Physical Constants

| Constant | Value | Unit | Used in |
|----------|-------|------|---------|
| `BCE_GRAVITY` | 9.80665 | m/s² | §11.3 |
| `BCE_OMEGA_EARTH` | 7.2921 × 10⁻⁵ | rad/s | §13 |
| `BCE_R_DRY_AIR` | 287.05 | J/(kg·K) | §3.3 |
| `BCE_STD_AIR_DENSITY` | 1.2250 | kg/m³ | §5.2 |
| `BCE_KELVIN_OFFSET` | 273.15 | K | §3 |
| `BCE_BALLISTIC_DRAG_CONSTANT` | 900.0 | — | §5.2 |
| `BCE_EXTERNAL_REFERENCE_DRAG_SCALE` | 0.84 | — | §11.3 |

### A.2 Integration Limits

| Constant | Value | Used in |
|----------|-------|---------|
| `BCE_MAX_RANGE_M` | 2500 | m | §11 |
| `BCE_MIN_VELOCITY` | 30.0 | m/s | §11.5 |
| `BCE_DT_MIN` | 10 µs | §11.4 |
| `BCE_DT_MAX` | 1 ms | §11.4 |
| `BCE_MAX_STEP_DISTANCE_M` | 0.25 | m | §11.4 |
| `BCE_ZERO_TOLERANCE_M` | 0.001 | m | §8 |
| `BCE_ZERO_MAX_ITERATIONS` | 50 | — | §8 |
| `BCE_MAX_SOLVER_ITERATIONS` | 500,000 | — | §11.5 |

### A.3 Unit Conversions

All conversions are in `bce_math_utils.h` namespace `bce::math`:
